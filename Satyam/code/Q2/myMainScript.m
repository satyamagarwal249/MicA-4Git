%% Q.2 (a)
%{
Strategy for populating the System Matrix :--

We first initialize the A matrix to be zero array of size
[no.of angles * size of 1 column of Radon] * [image rows * image columns]

-We took other array I (of size as of image)
-we put only one of element of I as 1 and take it Radon projection R. We
check which elements of R are non-zero for each theta. It signifies at
given angle, which rays pass though that pixel. We also find then degree of
affect of each pixels on bins of Radon by checking changes in intensity.
-We directly put that degree value at that corresponding row (where non-
zero radon and given theta) and column corresponding to that pixel.
-For faster determining the column no., we put the its address (instead of 1) as intenity value of the non-zero pixel of Image to
be projected 
E.g. if we take projection of 200th pixel (in vectorise image), we put its
intensity value as 200 and take radon. We find 3 values corresponding to
55,56,57 row of radon (let say for theta=0) as 50,100,50
Then we put the 
A[55][200]=0.25  (50/200)
A[56][200]=0.5   (100/200)   
A[57][200]=0.25  (50/200)
and similarly we do for all theta. The row no. depend on theta.

We repeat this one by one for pixel, making it only non-zero in image and
checking, which rows are affected in radon.
%}
